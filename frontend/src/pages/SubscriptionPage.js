  ...
  const startCheckout = async () => {
    ...
    try {
      ...
      if (result?.checkout_url) {
        toast.message("Abrindo checkout do Mercado Pago...");
        window.location.href = result.checkout_url; // mantém redirect na mesma aba
        return;
      }

      toast.error("Não foi possível abrir o checkout (checkout_url vazio).");
    } catch (err) {
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-zinc-400 mt-1">
          Plano do dono da academia: R$ 139,90/mês.
        </p>
      </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-1">
            <p className="font-semibold">
              Status atual:{" "}
              <span className="uppercase">{status?.status || "indefinido"}</span>
            </p>
            <p className="text-sm text-zinc-400">
              Trial até: {formatDate(status?.trial_ends_at)}
            </p>
            <p className="text-sm text-zinc-400">
              Período pago até: {formatDate(status?.current_period_end)}
            </p>
            <p className="text-sm text-zinc-400">
              Último pagamento: {formatDate(status?.last_payment_at)}
            </p>
            <p className="text-sm text-zinc-400">
              Grace até: {formatDate(status?.grace_until)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-md p-5 space-y-4">
        <h2 className="font-heading text-xl uppercase font-semibold flex items-center gap-2">
          <CreditCard className="w-5 h-5" /> Renovar assinatura
        </h2>

        <p className="text-sm text-zinc-400">
          O acesso ao sistema permanece bloqueado enquanto a assinatura estiver
          inativa.
        </p>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            disabled={refreshing}
            className="bg-zinc-800 text-white font-semibold uppercase tracking-wide text-sm h-11 px-6 rounded-sm hover:bg-zinc-700 disabled:opacity-50"
          >
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-md p-4 text-sm text-zinc-400 flex items-start gap-2">
        ...
        <p>
          Trial e período de carência são configuráveis por ENV no backend.
          Webhooks do Mercado Pago atualizam automaticamente o estado da
          assinatura.
        </p>
      </div>
    </div>
