...
import { api } from '../api';
import { ScanLine, CheckCircle, XCircle, RefreshCw } from 'lucide-react';
...
export default function AccessLogsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await api.listAccessLogs(100);
      setLogs(data);
    } catch {}
    setLoading(false);
  };
    ...
    <div data-testid="access-logs-page" className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Historico de acessos pela catraca</p>
        </div>
        <button data-testid="refresh-logs-btn" onClick={loadData}
          className="flex items-center gap-2 bg-zinc-800 text-white font-semibold uppercase tracking-wide text-sm px-6 py-2.5 rounded-sm hover:bg-zinc-700 transition-colors">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-md overflow-x-auto">
        <table data-testid="access-logs-table" className="w-full text-sm">
          ...
          <tbody>
            ...
            {logs.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-8 text-center text-zinc-500">Nenhum registro de acesso</td></tr>
            )}
          </tbody>
        </table>
      </div>
